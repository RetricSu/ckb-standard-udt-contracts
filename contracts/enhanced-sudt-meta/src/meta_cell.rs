...
use ckb_std::{
    ...
    error::SysError,
    high_level::{
        load_cell_data, load_cell_data_hash, load_cell_type, load_script, load_script_hash,
    },
};
...
pub fn validate_create(output_meta: &SudtMeta, meta_type_hash: &[u8; 32]) -> Result<(), Error> {
    if is_supply_tracked(output_meta.config_flags) {
        let initial_supply = sum_initial_udt_outputs(meta_type_hash)?;
...
pub fn sum_initial_udt_outputs(meta_type_hash: &[u8; 32]) -> Result<u128, Error> {
    let meta_code_hash: [u8; 32] = load_script()
    ...
    loop {
        ...
        if is_initial_udt_script(&type_script, meta_type_hash, &meta_code_hash)? {
...
fn is_initial_udt_script(
    ...
    if &code_hash == meta_code_hash {
        return Ok(false);
    }

    is_cell_dep_data_hash(&code_hash)
}

fn is_cell_dep_data_hash(code_hash: &[u8; 32]) -> Result<bool, Error> {
    let mut index = 0;
    loop {
        match load_cell_data_hash(index, Source::CellDep) {
            Ok(data_hash) if &data_hash == code_hash => return Ok(true),
            Ok(_) => index += 1,
